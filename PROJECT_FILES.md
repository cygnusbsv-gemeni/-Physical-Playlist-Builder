...
B12.4.4 - Minimal fused MP3 report/log presentation.

The project keeps the B12.4.3 minimal fused MP3 execution path. Eligible fused exports still run only when the effective output format is `mp3`, `settings.normalize_loudness=true`, `--skip-loudness` is not passed, and the run is not `--dry-run`.

B12.4.4 adds minimal fused report/log visibility without changing fused execution logic. `export_report.txt` now includes a `Fused MP3 Loudness Export` section with enabled/disabled status, source-measurement and output-location safety lines, encoded/failed counts, and the current fused verification limitation. `export_report.json` preserves existing per-track fused fields and adds compatible `fused_loudnorm_encoded` / `fused_loudnorm_failed` keys in totals/summary. `export.log` now may include two concise fused summary lines: `[fused-mp3] enabled` and `[fused-mp3] encoded=N failed=M`.

Fused processing logic, tag writing, M3U8 generation, dry-run behavior, copier safety boundaries, source-file safety, and the `physical_playlist_job.v1` contract remain unchanged. Fused results are still not re-measured or re-normalized by the regular loudness stage. Fused post-normalization verification remains skipped and is reported as skipped.

## Project File Map

| File path | Responsibility | Important notes |
|---|---|---|
| `README.md` | Project documentation and usage instructions | Must describe only implemented behavior. Updated for B12.4.4 minimal fused MP3 report/log presentation, validation caveat, safety rules, current limitations, and next-stage handoff. |
...
| `ppb/report.py` | Report generation for `export_report.json` and `export_report.txt` | B12.4.4 adds minimal fused MP3 report presentation without report schema redesign. It preserves existing per-track fused fields, adds compatible `fused_loudnorm_encoded` / `fused_loudnorm_failed` totals/summary keys, and writes a `Fused MP3 Loudness Export` section to `export_report.txt` with enabled/disabled status, encoded/failed counts, safety lines, and the current fused verification limitation. |
...
| `tests/test_loudness_processing.py` | Focused loudness tests | Includes B12.4.2 helper coverage, B12.4.3 fused execution coverage, and B12.4.4 fused report/log visibility coverage. B12.4.4 checks cover fused JSON/text/log presentation, fused encoded/failed counts, disabled fused section for non-fused reports, and non-fused `--skip-loudness` compatibility. |
...
|---|---|
| `ppb/report.py` | Added minimal fused MP3 report presentation. `export_report.txt` now includes `Fused MP3 Loudness Export`; `export_report.json` preserves fused per-track fields and adds compatible `fused_loudnorm_encoded` / `fused_loudnorm_failed` totals/summary keys. |
| `ppb/cli.py` | Added two concise fused summary lines to `export.log`: `[fused-mp3] enabled` and `[fused-mp3] encoded=N failed=M`. No execution logic was changed. |
| `tests/test_loudness_processing.py` | Expanded focused coverage for fused JSON/text/log presentation and non-fused compatibility. |
...
| `<final_output_dir>/export_report.json` | Machine-readable export report. B12.4.4 may include `totals.fused_loudnorm_encoded`, `totals.fused_loudnorm_failed`, and the same compatible keys in `summary`. Existing per-track fused fields are preserved. |
| `<final_output_dir>/export_report.txt` | Human-readable export report. B12.4.4 includes a `Fused MP3 Loudness Export` section with enabled/disabled status, source-measurement/output-location safety lines, encoded/failed counts, and fused verification skip note. |
| `<final_output_dir>/export.log` | Export log. B12.4.4 may include concise fused summary lines: `[fused-mp3] enabled` and `[fused-mp3] encoded=N failed=M`. |
...
| `C:\Temp\project_pytest\b12_4_2_loudness\` | Recommended external temp folder for focused B12.4.2 loudness helper tests. In one local Windows run, direct pytest setup/removal hit `PermissionError: [WinError 5]`; a diagnostic rerun with a temporary directory-permission workaround completed successfully. |
| `C:\temp\project_pytest\manual_b12_4_4\` | Manual diagnostic temp location used to verify B12.4.4 fused JSON/text/log presentation when the requested pytest command hit Windows temp/ACL issues. Not a project runtime folder. |
...
- Fused post-normalization verification remains skipped and is reported as skipped.
- B12.4.4 is a report/log presentation layer only; it does not change fused execution eligibility or FFmpeg processing.
- Fused processing logic, copier safety boundary, tag writing, M3U8 generation, dry-run behavior, and source-file safety remain unchanged.
- Source audio files are still never modified, tagged, normalized in-place, renamed, moved, or deleted.
- Fused results are still not re-measured or re-normalized by the regular loudness stage.
- Fused post-normalization verification remains skipped and is reported as skipped.
- `export_report.json` may contain extra fused totals/summary keys, but existing statuses and fields are preserved.
- `export_report.txt` and `export.log` are generated runtime files and should not be edited manually.
- B12.4.4 does not implement loudness cache, parallel processing, album normalization, ReplayGain/R128 tag-only mode, or changes to `physical_playlist_job.v1`.
